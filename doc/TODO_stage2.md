...
- [x] Refresh the consolidated report after updating its generator. `doc/build_report.py` now uses the measured 48/72 excluding-zero count with 8 degenerate cells, includes the completed CLIP paired-CI and `t*` analyses, and writes the stable output `doc/Report.pdf`.
