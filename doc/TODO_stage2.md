...
- [x] Explore the learned flow in reverse: start from class prototypes and integrate backward toward the encoder feature distribution. (Section 12 in both notebooks, now **quantitative as well as visual**: a per-class *recovery rate* asks whether each reverse-flowed prototype lands nearest to its own class's mean test feature, reported against the untouched forward prototype's recovery rate as the pre-transport reference, plus a five-panel strip showing where the prototypes travel at reverse `t` = 1, 0.75, 0.5, 0.25, 0. That reference is a ceiling only on the image branch; CLIP text prototypes can improve because they begin outside the image cloud. Caveats to state when presenting: the backward pass evaluates the field at `t` in {1, ..., 1/T} while the forward pass uses {0, ..., 1-1/T}, so it is not the exact inverse even for a perfectly learned field; and the forward flow is a deliberate contraction, so it is not invertible in principle - this recovers a plausible pre-image, not the original point.)
...
- [x] Run `05_flow_matching_clip.ipynb` end to end. **Done** - 81 velocity networks trained (108 result rows: the single standard network is reported at both `T`); the CLIP RN50 train/val splits were encoded once into the Stage 1 cache directory. Results in `doc/RESULTS.md` (Model 3). Headline: FM beats zero-shot in 32/36 cells. Against the same-supervision K-shot control, raw means favor FM in 8/36; paired 95% intervals support 6 FM wins, 28 control wins, and 2 no-effect cells, so the large Δ vs zero-shot measures the labels rather than the layer.

- [x] **Animated flow trajectories** (beyond the specification). Section 21 in both `04` and `05`: the two objectives side by side from an identical starting feature, one panel each, with an arrow showing the actual Euler update at every step, exported as embedded GIFs. The setting is selected programmatically (largest ΔAcc in `04`; largest gain over the K-shot control in `05`) rather than hardcoded, and the cell asserts `T+1` states, an identical start for both models, and no gradients. Read qualitatively - the transport happens in the full feature space and this is a 2-D projection of it.
...
- **`t = 1` is a convention, not an optimum.** Accuracy strictly peaks before the endpoint in all 6 representative CLIP-branch curves and 6 of 12 image-branch curves. Validation-selected `t*` helps in 18 of 36 image-branch conditions (mean +.0018, best +.0303) and in 17 of 18 CLIP-branch conditions (mean +.0239, best +.0520). On the image branch it converts the largest regression into a gain; on the CLIP branch it does not convert any control-relative loss into a win.
...
- **Most of the sub-1-point gains are not measurable.** The paired 95% CI in `04` §14 excludes zero in 48 of 72 cells - 24 of 24 on Aircraft, 7 of 24 on DTD, and 17 of 24 on Flowers-102. Eight Flowers-102 K=10 intervals are degenerate because there is only one effective subset, leaving 40 non-degenerate effects. None of the three mean regressions is significant. McNemar is significant far more often, but its unit is the test image rather than the run, so it ignores subset variance; quote the seed-level CI and flag the degenerate cells.
...
- [ ] Refresh two stale notebook Markdown notes after the rerun: `04` §14b still says 33/72 cells are unresolved even though the executed §14 output now says 24/72, and `05` §19b still frames the 28/36 control losses as resting only on differences of means even though §19 is now populated. The executed tables are the source of truth; no notebook source was changed in this documentation-only pass.
- [ ] Refresh `doc/Stage1_Stage2_Report.pdf` only after updating its generator. `doc/build_report.py` still hardcodes the pre-rerun 39/72/9-degenerate image-branch wording and says the CLIP paired CI and `t*` were not run. Running it unchanged would reproduce stale prose, so the PDF was deliberately not regenerated in this documentation-only pass.
...
- [x] Paired per-seed `ΔAcc` with 95% CIs plus McNemar on the shared test images (`04` §14), replacing difference-of-means. Now executed on **both** branches - `05` §19 pairs against the K-shot control, with `05` §9 saving the per-run control predictions McNemar needs. At `K=full` the Stage 1 prototype/control is a single deterministic run, so the spread there is FM initialization variance alone - stated in the table via `variance_source`.
...
- [x] Validation-selected stopping time `t*` (`04` §16). Selected on validation only; the test-optimal `t` is reported as an unreachable bound. The required table still classifies `ẑ_T`. **Measured** on both branches: `04` helps in 18 of 36 conditions, mean +0.0018, best +0.0303, and turns the DTD/ResNet-18 `full` rolled-out regression (-.0206) into +.0097 over the baseline. `05` §20 helps in 17 of 18 conditions, mean +.0239, best +.0520, but no tested condition crosses from below the K-shot control to above it.
...
- [x] Execute `05` §17-§21. **Done** - reverse flow, intermediate flow times, paired control analysis, validation-selected `t*`, and the animations all produced.
- [x] **Run `05` top to bottom** to execute §9 / §19 / §20. **Done** - per-run control predictions were saved; paired intervals and validation-selected stopping results are populated without retraining the FM grid.
...
- [ ] **`04` §14b and `05` §19b with `RUN_EXTENDED = True`** (about 540 and 270 velocity networks). The current `n=3` image-branch result leaves 24 of 72 cells unresolved; intervals should narrow roughly 3.4× at `n=10`. The required tables stay at `n=3`.
...
### Added 2026-08-24 execution status

Five additions came out of reviewing why the results read as weak. The `05` control-prediction, paired-CI, and validation-selected stopping additions have now run. The two gated supplements remain unexecuted.

- [x] `05` §9 saves per-run control predictions (blocker for §19's McNemar).
- [x] `05` §19 - paired ΔAcc vs the control, 95% CIs, McNemar.
- [x] `05` §20 - validation-selected `t*`, reported against the control as well as against `t=1`.
- [ ] `04` §14b / `05` §19b - extended-repetition supplement, seeds 0-9, separate `extended/` tree.
- [ ] `04` §6b - zero-init pilot. The identity-at-initialization guard passes, but `RUN_ZERO_INIT_PILOT` remained false in the rerun.
...
One reporting change the run forced, which must be carried into any presentation: **"improves in 66 of 72 cells" is a difference of means, while the paired 95% CI (§14) excludes zero in 48 of 72** - all 24 Aircraft cells, 7 of 24 on DTD, and 17 of 24 on Flowers-102. Eight of the Flowers-102 intervals are degenerate K=10 results, leaving 40 non-degenerate effects. None of the three mean regressions is significant. Quote the raw count, the paired count, and the degeneracy caveat together.
