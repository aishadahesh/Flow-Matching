...
Its numbers must never be merged into the Stage 2 table, for one specific reason: the FM layer trains on labelled pairs, so it converts zero-shot CLIP into a K-shot method, and `ΔAcc` against the zero-shot baseline measures the *labels* as much as the layer. That notebook therefore always reports three numbers together — zero-shot CLIP, the K-shot image-prototype control on the same CLIP features, and FM — and `delta_vs_control` is the only like-for-like comparison on that branch. Note what it does **not** have: no paired per-seed CI or McNemar (the image branch's §14) and no validation-selected `t*` (§16). Its §19 is the animation section. Since §14 materially changed how the image branch reads, "loses to the control in 28 of 36 cells" is still a difference of means and should be quoted as such.
